...
class MealPlans extends Table {
  ...
  TextColumn get breakfastJson => text().nullable()();
  TextColumn get lunchJson => text().nullable()();
  TextColumn get dinnerJson => text().nullable()();
  TextColumn get snacksJson => text().nullable()();
...
class MealPlansDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<MealPlan?> getCurrentWeekPlan(String userId) async {
    final now = DateTime.now();
    final weekStart = now.subtract(Duration(days: now.weekday - 1));
    final results = await (select(mealPlans)
          ..where((t) => t.userId.equals(userId) & t.weekStartDate.isBiggerOrEqualValue(weekStart)))
        .get();
    return results.isEmpty ? null : results.first;
  }
  ...
  Future<int> generatePlan({
    ...
    String? dosha,
    String cuisineType = 'North Indian',
  }) async {
    final breakfasts = _getBreakfastTemplates(cuisineType);
    final lunches = _getLunchTemplates(cuisineType);
    final dinners = _getDinnerTemplates(cuisineType);
    final snacks = _getSnackTemplates(cuisineType);
    
    final breakfastJson = _distributeCalories(breakfasts, tdee * 0.25, 7);
    final lunchJson = _distributeCalories(lunches, tdee * 0.35, 7);
    final dinnerJson = _distributeCalories(dinners, tdee * 0.30, 7);
    final snacksJson = _distributeCalories(snacks, tdee * 0.10, 7);
    
    return into(mealPlans).insert(
      MealPlansCompanion.insert(
        userId: userId,
        weekStartDate: _getWeekStart(),
        breakfastJson: Value(breakfastJson),
        lunchJson: Value(lunchJson),
        dinnerJson: Value(dinnerJson),
        snacksJson: Value(snacksJson),
  ...
  List<PlannedMeal> _getBreakfastTemplates(String cuisine) {
    final templates = <String, List<PlannedMeal>>{
      'North Indian': [
        const PlannedMeal('Paratha + Curd', 350, 'flatbread with yogurt'),
        const PlannedMeal('Poha', 280, 'flattened rice'),
        const PlannedMeal('Idli Sambar', 220, 'rice cakes with lentil soup'),
        const PlannedMeal('Paneer Sandwich', 320, 'cottage cheese sandwich'),
        const PlannedMeal('Banana Shake + Nuts', 280, 'protein shake'),
        const PlannedMeal('Besan Cheela', 250, 'gram flour pancake'),
        const PlannedMeal('Omelette + Toast', 300, 'eggs with bread'),
      ],
      'South Indian': [
        const PlannedMeal('Idli + Sambar', 280, 'rice cakes'),
        const PlannedMeal('Dosa', 320, 'fermented rice crepe'),
        const PlannedMeal('Upma', 260, 'semi-porridge'),
        const PlannedMeal('Pongal', 380, 'rice & lentil dish'),
        const PlannedMeal('Banana', 120, 'fruit'),
        const PlannedMeal('Podi Dosa', 350, 'spiced crepe'),
        const PlannedMeal('Ragi Porridge', 220, 'finger millet'),
      ],
      'Bengali': [
        const PlannedMeal('Luchi + Aloo Dum', 380, 'bread with potato'),
        const PlannedMeal('Chai + Paratha', 350, 'tea with bread'),
        const PlannedMeal('Muri +石榴', 280, 'puffed rice'),
        const PlannedMeal('Egg Toast', 300, 'eggs on bread'),
        const PlannedMeal('Sandesh', 180, 'cottage cheese sweet'),
        const PlannedMeal('Phulji', 260, 'fried bread'),
        const PlannedMeal('Roshogolla Curd', 250, 'sweet yogurt'),
      ],
      'Gujarati': [
        const PlannedMeal('Thepla + Curd', 320, 'flatbread with yogurt'),
        const PlannedMeal('Poha', 260, 'flattened rice'),
        const PlannedMeal('Handvo', 350, 'vegetable cake'),
        const PlannedMeal('Dhokla', 200, 'steamed snack'),
        const PlannedMeal('Fafda + Jalebi', 450, 'fried snack'),
        const PlannedMeal('Methi Thepla', 300, 'fenugreek flatbread'),
        const PlannedMeal('Mango Shake', 280, 'mango drink'),
      ],
    };
    return templates[cuisine] ?? templates['North Indian']!;
  }

  List<PlannedMeal> _getLunchTemplates(String cuisine) {
    final templates = <String, List<PlannedMeal>>{
      'North Indian': [
        const PlannedMeal('Dal + Roti + Sabzi', 550, 'lentils with bread'),
        const PlannedMeal('Paneer Biryani', 620, 'rice with cheese'),
        const PlannedMeal('Kadhi Chawal', 520, 'yogurt curry with rice'),
        const PlannedMeal('Rajma Chawal', 580, 'kidney beans'),
        const PlannedMeal('Chicken Curry + Roti', 650, 'meat with bread'),
        const PlannedMeal('Veg Pulao', 480, 'rice dish'),
        const PlannedMeal('Dal Makhani', 520, 'creamy lentils'),
      ],
      'South Indian': [
        const PlannedMeal('Sambar Rice', 520, 'lentil rice'),
        const PlannedMeal('Rasam Rice', 420, 'sour soup rice'),
        const PlannedMeal('Curry Leaves Chicken', 580, 'meat dish'),
        const PlannedMeal('Avial', 450, 'vegetable curry'),
        const PlannedMeal('Fish Fry + Rice', 550, 'fish with rice'),
        const PlannedMeal('Vegetable Ishtew', 480, 'stew'),
        const PlannedMeal('Coconut Rice', 520, 'rice with coconut'),
      ],
      'Bengali': [
        const PlannedMeal('Macher Jhol + Rice', 520, 'fish curry with rice'),
        const PlannedMeal('Dal + Roti', 450, 'lentils with bread'),
        const PlannedMeal('Kosha Mangsho', 620, 'mutton curry'),
        const PlannedMeal('Bhuna Khichuri', 480, 'spiced rice'),
        const PlannedMeal('Shukto', 380, 'bitter vegetable'),
        const PlannedMeal('Chicken Bhuna', 580, 'meat dish'),
        const PlannedMeal('Aloo Posto', 420, 'potato poppy'),
      ],
      'Gujarati': [
        const PlannedMeal('Kadhi + Roti', 480, 'yogurt curry'),
        const PlannedMeal('Paneer Pulao', 520, 'rice with cheese'),
        const PlannedMeal('Undhiyu', 420, 'mixed vegetable'),
        const PlannedMeal('Dal Dhokdi', 520, 'lentil dumplings'),
        const PlannedMeal('Bhindi Masala', 380, 'okra dish'),
        const PlannedMeal('Methi Malai Mutter', 450, 'fenugreek peas'),
        const PlannedMeal('Gujarati Thali', 650, 'full meal'),
      ],
    };
    return templates[cuisine] ?? templates['North Indian']!;
  }

  List<PlannedMeal> _getDinnerTemplates(String cuisine) {
    final templates = <String, List<PlannedMeal>>{
      'North Indian': [
        const PlannedMeal('Light Dal + Roti', 400, 'simple meal'),
        const PlannedMeal('Soup + Salad', 250, 'light option'),
        const PlannedMeal('Grilled Chicken', 350, 'protein'),
        const PlannedMeal('Vegetable Stir Fry', 280, 'light'),
        const PlannedMeal('Curd + Fruits', 200, 'light'),
        const PlannedMeal('Egg Curry + Roti', 420, 'eggs'),
        const PlannedMeal('Oatmeal', 250, 'porridge'),
      ],
      'South Indian': [
        const PlannedMeal('Rasam', 150, 'soup'),
        const PlannedMeal('Vegetable Poriyal', 250, 'stir fry'),
        const PlannedMeal('Fish Curry + Rice', 450, 'fish'),
        const PlannedMeal('Buttermilk + Salad', 200, 'drink'),
        const PlannedMeal('Appalam', 180, 'crackers'),
        const PlannedMeal('Keerai', 220, 'greens'),
        const PlannedMeal('Coconut Milk', 180, 'drink'),
      ],
      'Bengali': [
        const PlannedMeal('Bhapa P在内er', 350, 'steamed fish'),
        const PlannedMeal('Lau Ghonto', 320, 'bottle gourd'),
        const PlannedMeal('Chingri Malai', 380, 'prawns'),
        const PlannedMeal('Light Dal', 280, 'simple lentils'),
        const PlannedMeal('Cabbages', 250, 'vegetable'),
        const PlannedMeal('Misti Doi', 220, 'sweet yogurt'),
        const PlannedMeal('Phuchka', 280, 'crispy balls'),
      ],
      'Gujarati': [
        const PlannedMeal('Kadhi', 200, 'yogurt soup'),
        const PlannedMeal('Fajita', 250, 'light'),
        const PlannedMeal('Methi', 220, 'greens'),
        const PlannedMeal('Thepla', 320, 'flatbread'),
        const PlannedMeal('Salad', 150, 'vegetables'),
        const PlannedMeal('Chaas', 120, 'buttermilk'),
        const PlannedMeal('Light Sabzi', 280, 'vegetable'),
      ],
    };
    return templates[cuisine] ?? templates['North Indian']!;
  }

  List<PlannedMeal> _getSnackTemplates(String cuisine) {
    return const [
      PlannedMeal('Nuts', 150, 'almonds/walnuts'),
      PlannedMeal('Fruit', 80, 'apple/banana'),
      PlannedMeal(' roasted chickpeas', 120, 'protein'),
      PlannedMeal('Greek Yogurt', 100, 'probiotic'),
      PlannedMeal('Boiled Egg', 70, 'protein'),
      PlannedMeal('Smoothie', 180, 'drink'),
      PlannedMeal(' Roasted makhana', 100, 'fox nuts'),
    ];
  }

  String _distributeCalories(List<PlannedMeal> meals, double dailyCals, int days) {
    final result = <Map<String, dynamic>>[];
    final perMeal = (dailyCals / 3).round();
    
    for (int day = 0; day < days; day++) {
      final dayMeals = meals.sublist((day * 3) % meals.length, ((day * 3) + 3) % meals.length + 1);
      if (dayMeals.isEmpty) continue;
      
      result.add({
        'day': day,
        'meals': dayMeals.map((m) => {'name': m.name, 'calories': m.calories, 'desc': m.desc}).toList(),
      });
    }
  ...
  Future<List<String>> generateGroceryList(MealPlan plan) async {
    final groceries = <String>[];
    
    if (plan.breakfastJson != null) {
      groceries.addAll(['Paneer', 'Milk', 'Curd', 'Eggs', 'Bread', 'Ghee']);
    }
    if (plan.lunchJson != null || plan.dinnerJson != null) {
      groceries.addAll([
        'Rice', 'Wheat Flour', 'Dal (Lentils)', 'Vegetables',
        'Ginger', 'Garlic', 'Onions', 'Tomatoes', 'Oil',
        'Spices', 'Cumin', 'Coriander', 'Turmeric',
      ]);
    }
    
    return groceries.toSet().toList()..sort();
  }
}
...
class PlannedMeal {
  ...
  const PlannedMeal(this.name, this.calories, this.desc);
}
